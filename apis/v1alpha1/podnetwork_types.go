...
import (
	drav1beta1 "k8s.io/api/resource/v1beta1"
...
type PodNetworkSpec struct {
	...
	// Config specifies provider implementation specific configuration.
	// Config cannot be configured with NamespaceSelector.
	//
	// +optional
	Config *drav1beta1.DeviceConfiguration `json:"config,omitempty"`

	// DeviceClassName specifies implementation provided class.
	//
	// You need to specify oneOf: [DeviceClassName, NamespaceSelector].
	// Which classes are available depends on the implementation.
	//
	// Implementations may use this to restrict which devices may get
	// requested by only installing classes with selectors for permitted
	// devices. If users are free to request anything without restrictions,
	// then implementations can create an empty DeviceClass for users
	// to reference.
	//
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=253
	...
	// NamespaceSelector specifies labels to select namespaces in
	// which we expect to find existing ResourceClaimTemplates
	// named the same as this PodNetwork.
	//
	// You need to specify oneOf: [DeviceClassName, NamespaceSelector].
	//
	// +optional
